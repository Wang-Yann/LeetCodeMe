...
class Master:
    ...
    def guess(self, word: str) -> int:
        if word not in self.wordlist:
            return -1
        return sum(self.secret[i] == word[i] for i in range(6))
...
def test_solutions():
    ...
    assert master.guess("aaaaaa") == -1  # 因为 "aaaaaa" 不在 wordlist 中.
    assert master.guess("acckzz") == 6  # 因为 "acckzz" 就是秘密，6个字母完全匹配。
    assert master.guess("ccbazz") == 3  # 因为 "ccbazz" 有 3 个匹配项。
    assert master.guess("eiowzz") == 2  # 因为 "eiowzz" 有 2 个匹配项。
    assert master.guess("abcczz") == 4  # 因为 "abcczz" 有 4 个匹配项。
    assert Solution().findSecretWord(wordlist, master) is None

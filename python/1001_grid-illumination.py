    ...
    def gridIllumination(self, N: int, lamps: List[List[int]], queries: List[List[int]]) -> List[int]:
        ...
        for r, c in queries:
            ...
            for i in [-1, 0, 1]:
                for j in [-1, 0, 1]:
                    nr, nc = r + i, c + j
                    if not (0 <= nr <= N - 1 and 0 <= nc <= N - 1 and (nr, nc) in lookup):
                        continue
                    lookup.remove((nr, nc))
                    row[nr] -= 1
                    col[nc] -= 1
...
@pytest.mark.parametrize("kw,expected", [
    [dict(N=5, lamps=[[0, 0], [4, 4]], queries=[[1, 1], [1, 0]]), [1, 0]],
    [dict(N=5, lamps=[[0, 0], [0, 4]], queries=[[0, 4], [0, 1], [1, 4]]), [1, 1, 0]],
])
def test_solutions(kw, expected):
    assert Solution().gridIllumination(**kw) == expected

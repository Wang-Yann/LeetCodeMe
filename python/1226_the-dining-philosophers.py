...
# -*- coding: utf-8 -*-
# @Author        : Rock Wayne 
# @Created       : 2020-07-04 22:03:07
# @Last Modified : 2020-07-04 22:03:07
...
@pytest.mark.parametrize("DiningPhilosophersCls", [DiningPhilosophers])
# @pytest.mark.parametrize("DiningPhilosophersCls", [DiningPhilosophersLock ])
@pytest.mark.parametrize("args,expected", [
    (1, [[4, 2, 1], [4, 1, 1], [0, 1, 1], [2, 2, 1], [2, 1, 1], [2, 0, 3],
...
def test_solutions(capsys, args, expected, DiningPhilosophersCls):
    foo = DiningPhilosophers()
    with capsys.disabled():
        print("Begin Class: {}".format(foo.__class__.__name__))

    threads = []
    for i in range(5):
        t = Thread(target=foo.wantsToEat, args=(i, pickLeftFork, pickRightFork, eat, putLeftFork, putRightFork))
        t.start()
    for t in threads:
        t.join()

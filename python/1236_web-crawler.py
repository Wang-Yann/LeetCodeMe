...
# -*- coding: utf-8 -*-
# @Author        : Rock Wayne 
# @Created       : 2020-08-06 14:05:55
# @Last Modified : 2020-08-06 14:05:55
...
class Solution:
    def crawl(self, startUrl: str, htmlParser: 'HtmlParser') -> List[str]:
        """AC"""
        ans = set()
        domain_parts = parse.urlparse(startUrl)
        domain = "{}://{}".format(domain_parts.scheme, domain_parts.netloc)
        dq = collections.deque([startUrl])
        while dq:
            node = dq.popleft()
            ans.add(node)
            for neighbor in htmlParser.getUrls(node):
                if neighbor not in ans and neighbor.startswith(domain):
                    dq.append(neighbor)
        return sorted(ans)

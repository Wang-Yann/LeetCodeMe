...
class UndergroundSystem:
    ...
    def __init__(self):
        self.trip_hists = collections.defaultdict(list)
        self.user_trace = collections.defaultdict(list)

    def checkIn(self, id: int, stationName: str, t: int) -> None:
        self.user_trace[id].append((t, stationName))

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start_time, start_pos = self.user_trace[id].pop()
        self.trip_hists[(start_pos, stationName)].append(t - start_time)

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        data = self.trip_hists[(startStation, endStation)]
        if data:
...
def test_solution():
    ...
    # 返回 14.0。从 "Paradise"（时刻 8）到 "Cambridge"(时刻 2 行程只有一趟
    assert undergroundSystem.getAverageTime("Leyton", "Waterloo") == 11.0
    # 返回 11.0。总共 有 2 躺从 "Leyton" 到 "Waterloo" 的行程，编号为 id=45 的乘客出发于 time=3 到达于 time=15，
    # 编号为 id=27 的 乘客于 time=10 出发于 time=20 到达。所以平均时间为 ( (15-3) + (20-10) ) / 2 = 11.0
    undergroundSystem.checkIn(10, "Leyton", 24)
    assert undergroundSystem.getAverageTime("Leyton", "Waterloo") == 11.0  # 返回 11.0
    undergroundSystem.checkOut(10, "Waterloo", 38)
    assert undergroundSystem.getAverageTime("Leyton", "Waterloo") == 12.0  # 返回 12.0

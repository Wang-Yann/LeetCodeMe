...
# -*- coding: utf-8 -*-
# @Author        : Rock Wayne 
# @Created       : 2021-02-22 22:40:37
# @Last Modified : 2021-02-22 22:40:37
    ...
    def numsGame(self, nums: List[int]) -> List[int]:
        ...
        for i in range(1, len(new_nums)):
            if i % 2 == 0:
                ...
                if new_nums[i] < left_root:
                    median = left_root
                    heapq.heapreplace(left_heap, -new_nums[i])  # 由于heaqp是小根堆，存相反数来实现大根堆
                    sl = sl - left_root + new_nums[i]
                elif new_nums[i] > right_root:
                    median = right_root
                    heapq.heapreplace(right_heap, new_nums[i])
                    sr = sr - right_root + new_nums[i]
                else:
                    median = new_nums[i]

            else:
                if new_nums[i] > median:
                    heapq.heappush(right_heap, new_nums[i])
                    heapq.heappush(left_heap, -median)
                    sl += median
                    sr += new_nums[i]
                else:
                    heapq.heappush(left_heap, -new_nums[i])
...
@pytest.mark.parametrize("kwargs,expected", [
    [dict(nums=[3, 4, 5, 1, 6, 7]), [0, 0, 0, 5, 6, 7]],
    [dict(nums=[1, 2, 3, 4, 5]), [0, 0, 0, 0, 0]],
    [dict(nums=[1, 1, 1, 2, 3, 4]), [0, 1, 2, 3, 3, 3]],
...
def test_solutions(kwargs, expected, SolutionCLS):
    assert SolutionCLS().numsGame(**kwargs) == expected

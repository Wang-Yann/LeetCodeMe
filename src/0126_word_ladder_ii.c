...
char ***findLadders(char *beginWord, char *endWord, char **wordList, int wordListSize, int *returnSize,
    ...
    *returnSize = 0;
    ...
    //BFS
    int *cur = (int *) malloc(sizeof(int) * size); // curr level in BFS
    int *prev = (int *) malloc(sizeof(int) * size);// prev level in BFS
    int prevSize, curSize = 1;
    int *curVisited = (int *) malloc(sizeof(int) * size); // to make sure curr members are unique
    int level = 1; // level of ladder
    cur[0] = 0;
    visited[0] = 1;
    int retlevel = 0; // ladder size, marks the end of BFS
    while (retlevel == 0 && curSize > 0) {
        level++;
        ...
        for (int i = 0; i < prevSize; ++i) {
            for (int j = 0; j < diffSize[prev[i]]; ++j) {
                ...
                // find the lowest ladder level, finish current BFS
                if (k == endidx) {
                    retlevel = level;
                }
                // no duplicates in curr list
                if (curVisited[k]) {
                    continue;
                }
                cur[curSize++] = k;
        ...
        // set visited after search through one level
